...
// Idea 2: a fenwick tree consists of layers of intervals. The first layer is unique: You take the floor of log base 2(the length of the entire array). Call that value X.You then take the interval 0 - X, interval format [l, r), and set that as an interval for the Fenwick tree. Next you "cut off" the first X items from the interval and you repeat the process for the interval starting at X instead of 0, and rinse and repeat until you've divided up the ENTIRETY of the array into intervals. Now for the remaining layers: check the layer below it. That is the active layer. From each interval in the active layer, take them as a subtask. Note that all subtasks have a length which is a power of 2. Fill them up left to right as follows: half length, quarter length, eighth length, sixteenth length... etc. until you reach an interval of length zero (essentially, no length). These new intervals are the subtasks for the new layer. Repeat this process for all active subtasks. 
// Oh yeah, and for both ideas: add a final layer which just consists of 1-unit intervals.
